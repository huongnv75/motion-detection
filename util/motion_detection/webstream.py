...
class WebStream:
    ...
    def make_path(self, prefix, suffix, filename):
        now = datetime.utcnow()
    ...
    def active_motion(self):
        ...
        if motion_config and motion_config.replace('\\', '').replace('"x"', "'x'").replace('"y"', "'y'") != '[]':
            ...
            print("=========================================================================")
            print("motion_confi==========>", motion_config)
        ...
        while True:
            ...
            if count_frame == 0 and count_milisecond == 0:
                ...
                frame = imutils.resize(frame,
                                       width=int(width * int(motion_config['detect_resolution']) / 100))
                height, width = frame.shape[:2]
                if check_region == 0:
                ...
                if total > 0:
                    ...
                    if motion is not None:
                        ...
                        if start_time == '':
                            ...
                            image_file = self.make_path(prefix, suffix, "thumbnail")
                            self.create_file(image_file)
